...
/* define('BOT_UNIQUE_NAME', 'bot_it_request');

//Подключаем хранилище и его свойства
...
$result = restCommand('imbot.register', Array(
	...
	'NAME' => BOT_UNIQUE_NAME,
        'COLOR' => 'GREEN',
        'EMAIL' => 'it@estelab.ru',
        'PERSONAL_BIRTHDAY' => '2021-02-14',
        'WORK_POSITION' => 'Если не знаю как сделать, то вместе разберемся',
        'PERSONAL_WWW' => 'http://corp.estelab.ru',
        'PERSONAL_GENDER' => 'M'
    )
), $_REQUEST["auth"]);
$botId = $result['result'];
...
registerCommand($botId, 'create', 'Y', 'Заявка создана', $handlerBackUrl);
registerCommand($botId, 'delete', 'N', 'Удаление диалога', $handlerBackUrl);
...
$result = restCommand('imbot.register', array(
    ...
    'PROPERTIES'            => array( // Личные данные чат-бота (обяз.)
        ...
        'WORK_POSITION'     => 'Помошник для заявок ИТ',
...
writeToLog(registerCommand($bot_ID, 'create', 'Y', 'Создать заявку ИТ', $handlerBackUrl), "создание команды");
writeToLog(registerCommand($bot_ID, 'cancel', 'Y', 'Отмена заявки', $handlerBackUrl), "создание команды");
